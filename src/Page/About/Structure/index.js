...
function Structure() {
  ...
  const onDownload = () => {
    ...
    link.download =
      lang == "ja"
        ? "Astro Coding Go_カリキュラム.pdf"
        : "Astro Coding Go_Curriculum.pdf";
    link.href = file;
    link.click();
  };
  ...
  return (
    <div className="structure">
      <h3 className="structure__title">
        <FormattedMessage id="ID_ABOUT_STRUCTURE_TITLE" />
      </h3>
      ...
      {bCurriculum.map((curriculum, index) => (
        <div key={curriculum.course.id} className="structure__slide-box">
          <div className="structure__slide-flex-box">
            <img
              alt="강의 썸네일 임시"
              src={URL.S3_DREAMCLASS + curriculum.course.smallPosterURL}
            />
            <div className="structure__content-title-box">
              <h3 className="structure__content-title">
                {curriculum.course.title}
              </h3>
              <span className="structure__mission-length">
                {curriculum.course.progress.net}{" "}
                <FormattedMessage id="ID_COMMON_MISSION" />
              </span>
              <span className="structure__time">
                {" "}
                · {curriculum.course.studyEsmtMinute + "min"}
              </span>
            </div>
            ...
            <button
              className={
                curriculum.isOpen
                  ? "structure__slide-btn active"
                  : "structure__slide-btn"
              }
              type="button"
          ...
          <div
            className={
              curriculum.isOpen
                ? "structure__slide-inner-wrap show"
                : "structure__slide-inner-wrap"
            }
          >
            <ul className="structure__slide-inner-list">
              {curriculum.projects.map((project) => (
                <li key={project.id} className="structure__slide-inner-item">
                  <span className="structure__mission-label">
                    {project.label}
                  </span>
