...
class DreamSlide extends Component {
  ...
  shouldComponentUpdate(nextProps) {
    ...
    if (
      this.props.shouldRerender !== nextProps.shouldRerender &&
      ...
      <div className="dreamslide" ref={this.props.slideRef}>
        {this.props.markdown ? (
          <ReactMarkdown
            skipHtml={false}
            allowDangerousHtml
            astPlugins={[htmlParserWith({ getSpriteIcon: this.getSpriteIcon })]}
            renderers={{
              code: (props) => (
                <CodeRenderer
                  {...props}
                  projectId={this.state.projectId}
                  getSpriteIcon={this.getSpriteIcon}
                />
              ),
            }}
          >
